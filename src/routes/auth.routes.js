...
authRouter.post(
  ...
  wrap((req, res) => {
    ...
    if (db.prepare(`SELECT id FROM users WHERE contact = ?`).get(contact)) throw bad('Этот контакт уже зарегистрирован');
    ...
    logActivity(user.id, user.id, 'register', contact);
...
authRouter.post(
  ...
  wrap((req, res) => {
    requireFields(req.body, ['contact', 'password']);
    const contact = normalizeContact(req.body.contact);
    const user = contact ? db.prepare(`SELECT * FROM users WHERE contact = ?`).get(contact) : null;
